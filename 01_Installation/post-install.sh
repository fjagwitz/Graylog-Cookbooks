#!/bin/bash
GL_GRAYLOG_ADMIN=$(cat /opt/graylog/your_graylog_credentials.txt | awk -F'"' '{print $2}')
GL_GRAYLOG_PASSWORD=$(cat /opt/graylog/your_graylog_credentials.txt | awk -F'"' '{print $4}')

# Checking Graylog License
#echo "[INFO] - CHECK GRAYLOG LICENSE "
#active_license=$(curl -s http://localhost/api/plugins/org.graylog.plugins.license/licenses/status?only_legacy=false -u "${GL_GRAYLOG_ADMIN}":"${GL_GRAYLOG_PASSWORD}" | jq .status | jq -r .[0].license.subject)
#
#if [ $active_license != "/license/enterprise" ]
